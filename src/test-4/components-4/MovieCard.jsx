const MovieCard = ({setFormShow,...movie}) => {
  const rating = movie.vote_average;
  const popular = movie.popularity;

  function handleEdit() {
    setFormShow({ action: true, editId: movie.id, deleteId: null });
  }

  function handleDelete() {
    setFormShow({ action: true, editId: null, deleteId: movie.id });
  }

  return (
    <div className="h-120 brightness-70 relative">
      <img
        src={
          movie.poster_path 
      ...
      <div className="absolute flex-col space-y-3 text-white/90 font-semibold bg-black/80 top-0 right-0 left-0 bottom-0 flex items-center justify-center opacity-0 text-div hover:opacity-100 transition-all duration-300">
        ...
        <p className="w-[90%] line-clamp-10">{movie.desc ?? movie.desc}</p>
        <div className="absolute flex space-x-3 bottom-3 right-3 *:hover:border *:rounded *:p-2 *:cursor-pointer *:hover:**:fill-orange-600 *:active:*:-rotate-12 *:border-orange-600">
          ...
          <button typeof="button" className="" onClick={handleDelete}>
